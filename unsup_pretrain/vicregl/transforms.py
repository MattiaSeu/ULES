...
class MultiCropTrainDataTransform(object):
    ...
    def __call__(self, main_img, complementary_img: Optional[torch.Tensor] = None):

        multi_crops = []
        complementary_crops = [] if complementary_img else []
        locations = []

        for i, crop_transform in enumerate(self.random_resized_crops):
            crop_data = self.process_crops(crop_transform, main_img, complementary_img, i)

            multi_crops.append(crop_data['main_crop'])
            if complementary_img:
                complementary_crops.append(crop_data['complementary_crop'])

            if self.return_location_masks:
                grid_size = 7 if i < self.num_crops[0] else 3
                flip_flag = not self.no_flip_grid and crop_data['is_flip']
                locations.append(_location_to_NxN_grid(crop_data['location'], grid_size, flip=flip_flag))

        if self.return_location_masks:
            return (multi_crops, complementary_crops), locations

        return multi_crops, complementary_crops


    def process_crops(self, crop_transform, main_img, complementary_img, index):
        if self.return_location_masks:
            crop, location = crop_transform(main_img)
            i, j, h, w, *_ = location
            comp_crop = FT.resized_crop(complementary_img, i, j, h, w, crop_transform.size, InterpolationMode.BICUBIC) if complementary_img else (None, None)
        else:
            # this does not work as I could not find a way to properly reproduce the random crop without locations
            rng_state = torch.get_rng_state()
            crop = crop_transform(main_img)
            torch.set_rng_state(rng_state)
            comp_crop = crop_transform(complementary_img) if complementary_img else None
            location = None


        crop, is_flip = self.flip(crop)
        if comp_crop and is_flip:
            comp_crop = FT.hflip(comp_crop)

        processed_main_crop = self.augmentations[index](crop) if crop.mode != "L" else transforms.ToTensor()(crop)
        processed_comp_crop = None
        if complementary_img and comp_crop:
            processed_comp_crop = self.augmentations[index](comp_crop) if comp_crop.mode != "L" else transforms.ToTensor()(
                comp_crop)
        # print_tensor(processed_main_crop)
        # print_tensor(processed_comp_crop)

        return {
            'main_crop': processed_main_crop,
            'complementary_crop': processed_comp_crop,
            'location': location,
            'is_flip': is_flip
        }
