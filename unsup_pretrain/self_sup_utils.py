...
def get_moco_dataset(hparams: ModelParams) -> DatasetBase:
    ...
    elif hparams.dataset_name == "cityscapes":
        return CityDataContrastive()
    else:
...
def get_class_dataset(name: str) -> DatasetBase:
    if name == "stl10":
        ...
        return Cityscapes(transforms=tra, root=city_data_path, split='train', mode='fine',
...
class MLP(torch.nn.Module):
    ...
    def forward(self, x):
        if isinstance(x, OrderedDict):
            ...
            return self.net(x.squeeze(0) if len(x.shape) == 4 else x)


def get_encoder(name: str, dataset: str, batch_size: int, num_classes: int,
    ...
    if name in ws_resnet.__dict__:
        ...
        model = torchvision.models.segmentation.fcn_resnet50(pretrained=False, progress=True, num_classes=num_classes,
                                                             aux_loss=None)
        return model
        # backbone_only = CustomLayer(model.backbone)
        # return backbone_only
        # return nn.Sequential(backbone_only, torch.nn.AdaptiveAvgPool2d(output_size=(1, 1)))
